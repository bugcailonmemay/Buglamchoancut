...
phone = input(f"[</>] 8==> Nhập SDT: ")
time.sleep(2)
if phone == "0822771058":
    print(f"[</>] 8==> Spam Làm Chó - Lêu Lêu 😝")
    exit()
delay = float(input(f"[</>] 8==> Nhập Số Lần Spam [Max 10]: "))
print(f"[</>] 8==> Khởi Tạo Luồng Spam => Kết Nối Sever => Thành Công")
time.sleep(3)
print(f"[</>] 8==> Đang Bắt Đầu Chạy Spam Sms + Call")
...
for i, thread in enumerate(threads, 1):
    ...
    print(f"| Phone [{phone}] | {current_time} | Trạng Thái => Thành Công | Số Lần {i} |")
    thread.start()
